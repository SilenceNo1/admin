 ...
 * 
 * @author FH 13161618211 作者单位： 联系方式： 创建时间：2014年7月10日
 * @version 1.0
 */
@Controller
...
public class WeixinController extends BaseController {
					...
					if (null != msgpd) {
						...
						if (null != msgpd) {
							...
							if (null != msgpd.getString("TITLE1") && null != msgpd.getString("IMGURL1")) {
								...
								if (null != msgpd.getString("TITLE2") && null != msgpd.getString("IMGURL2") && !"".equals(msgpd.getString("TITLE2").trim())
								...
								if (null != msgpd.getString("TITLE3") && null != msgpd.getString("IMGURL3") && !"".equals(msgpd.getString("TITLE3").trim())
								...
								if (null != msgpd.getString("TITLE4") && null != msgpd.getString("IMGURL4") && !"".equals(msgpd.getString("TITLE4").trim())
								...
								if (null != msgpd.getString("TITLE5") && null != msgpd.getString("IMGURL5") && !"".equals(msgpd.getString("TITLE5").trim())
								...
								if (null != msgpd.getString("TITLE6") && null != msgpd.getString("IMGURL6") && !"".equals(msgpd.getString("TITLE6").trim())
								...
								if (null != msgpd.getString("TITLE7") && null != msgpd.getString("IMGURL7") && !"".equals(msgpd.getString("TITLE7").trim())
								...
								if (null != msgpd.getString("TITLE8") && null != msgpd.getString("IMGURL8") && !"".equals(msgpd.getString("TITLE8").trim())
							...
							if (null != msgpd) {
								...
								JSONObject jsonObject = JSONObject.fromObject(AliyunUtils.artificialIntelligence(getmsg)).getJSONObject("result");
								Msg4Text rmsg = new Msg4Text();
								rmsg.setFromUserName(toUserName);
								rmsg.setToUserName(fromUserName);
								rmsg.setContent(jsonObject.getString("content"));
								session.callback(rmsg);
							}
						}
					}
	...
	public void getAt(PrintWriter out) {
		...
		try {
			...
			PrintWriter pw;
			try {
				pw = new PrintWriter(new FileWriter("/Users/silence/Downloads/access_token.txt"));
