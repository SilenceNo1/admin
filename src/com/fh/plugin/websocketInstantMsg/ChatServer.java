...
public class ChatServer extends WebSocketServer{
	...
	public void onMessage(WebSocket conn, String message){
		...
		if(null != message && message.startsWith("silenceadminqq13161618211")){
			this.userjoin(message.replaceFirst("silenceadminqq13161618211", ""),conn);
		}if(null != message && message.startsWith("Leavesilenceadminqq13161618211")){
			this.userLeave(conn);
		}if(null != message && message.contains("silenceadmin886")){
			String toUser = message.substring(message.indexOf("silenceadmin886")+10, message.indexOf("fhsilenceadmin888"));
			message = message.substring(0, message.indexOf("silenceadmin886")) +"[私信]  "+ message.substring(message.indexOf("fhsilenceadmin888")+12, message.length());
